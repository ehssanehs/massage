...
### اگر پیام «ایمیل یا رمز عبور اشتباه است» می‌گیرید

اگر قبلاً پروژه را نصب کرده‌اید و ورود با حساب پیش‌فرض کار نمی‌کند، هش رمز عبور داخل دیتابیس شما ناقص ذخیره شده است.
کافی است سیدر را دوباره اجرا کنید تا رمز پیش‌فرض بازنویسی شود:

```bash
php bin/console seed
```

یا رمز دلخواه خود را مستقیم تنظیم کنید:

```bash
php bin/console user:password admin@example.com MyNewPassword
```
