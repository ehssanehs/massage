...
function options(string $type): array { return match($type){
 'customer'=>array_map(fn($r)=>['id'=>$r['id'],'label'=>$r['label'],'balance'=>(float)($r['credit_balance']??0)], DB::select("SELECT id, CONCAT(first_name,' ',last_name,' - ',mobile) label, COALESCE(credit_balance,0) credit_balance FROM customers WHERE deleted_at IS NULL ORDER BY id DESC LIMIT 500")),
